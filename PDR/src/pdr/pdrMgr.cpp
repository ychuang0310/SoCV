...
bool PDRMgr::recursiveBlockCube(TCube s0){
  ...
  map< Cube*, pair<bool*, Cube*> > traceMap;
  Q.push(s0);
  traceMap[s0._cube] = make_pair(Z->getSATInputAssignment(), (Cube*)0);
  if (debug) {
  ...
  while (!Q.empty()) {
    ...
    if (s._frame == 0){
      ...
      Cube* c = s._cube;
      while(c != 0){
        cex.push_back(vector<bool>());
        cex.back().assign(traceMap[c].first, traceMap[c].first+Z->_I);
        c = traceMap[c].second;
      }
      for(map< Cube*, pair<bool*, Cube*> >::iterator it=traceMap.begin(); it!=traceMap.end(); ++it){
        delete it->first;
    ...
    if (!isBlocked(s)) {
      ...
      if (z._frame != -1) {
        ...
        z._frame = s._frame - 1;
        Q.push(z);
        traceMap[z._cube] = make_pair(Z->getSATInputAssignment(), s._cube);
        if (debug) {
          cerr << "pushed : " << z._cube << ", ";
          z._cube->show();
        }
...
TCube PDRMgr::generalize(TCube s) {
  ...
  for (unsigned i = 0; i < L; ++i) {
    ...
    if (tc->_latchValues[i]._dontCare == 1) {
      delete tc;
      continue;
    }
