...
void V3SvrPDRSat::mySimulation(const V3NetVec& dfsList, vector<bool>& values) {
  for(unsigned i=0; i<dfsList.size(); ++i){
    const V3NetId& n = dfsList[i];
    if(_ntk->getGateType(n) == AIG_NODE){
      bool in0 = values[_ntk->getInputNetId(n, 0).id];
      bool in1 = values[_ntk->getInputNetId(n, 1).id];
      if(_ntk->getInputNetId(n, 0).cp) in0 = !in0;
      if(_ntk->getInputNetId(n, 1).cp) in1 = !in1;
      values[n.id] = in0 & in1;
    }
  }
}
...
Cube* V3SvrPDRSat::ternarySimulation(Cube* c, bool b, bool* input) {
  ...
  if (dfsList.empty()) dfs(dfsList);

  // get right value
  vector<bool> values_ref(_ntk->getNetSize());
  for(unsigned i=0; i<_I; ++i) values_ref[_ntk->getInput(i).id] = input[i];
  for(unsigned i=0; i<_L; ++i) values_ref[_ntk->getLatch(i).id] = c->_latchValues[i]._bit;
  mySimulation(dfsList, values_ref);

  initValue3Data();
  queue<V3NetId> QQ;
  if(b){
    for(unsigned i=0; i<_L; ++i){
      const V3NetId& n = _ntk->getInputNetId(_ntk->getLatch(i), 0);
      _Value3List[n.id]._bit = values_ref[n.id];
      _Value3List[n.id]._dontCare = false;
      QQ.push(n);
    }
  }
  else{
    _Value3List[_monitor.id]._bit = values_ref[_monitor.id];
    _Value3List[_monitor.id]._dontCare = false;
    QQ.push(_monitor);
  }
  ...
  while(!QQ.empty()){
    V3NetId n = QQ.front();
    QQ.pop();
    if (_ntk->getGateType(n) != AIG_NODE) continue;

    const V3NetId& in0 = _ntk->getInputNetId(n, 0);
    const V3NetId& in1 = _ntk->getInputNetId(n, 1);
    if(_Value3List[n.id]._bit){
      // n = 1
      if(_Value3List[in0.id]._dontCare){
        _Value3List[in0.id]._bit = !in0.cp;
        _Value3List[in0.id]._dontCare = false;
        QQ.push(in0);
      }
      if(_Value3List[in1.id]._dontCare){
        _Value3List[in1.id]._bit = !in1.cp;
        _Value3List[in1.id]._dontCare = false;
        QQ.push(in1);
      }
    }
    else{
      // n = 0
      if(!_Value3List[in0.id]._dontCare && !_Value3List[in1.id]._dontCare){
        continue;
      }
      else if(_Value3List[in0.id]._dontCare && _Value3List[in1.id]._dontCare){
        // _Value3List[in0.id]._bit = values_ref[in0.id];
        // _Value3List[in1.id]._bit = values_ref[in1.id];
        // _Value3List[in0.id]._dontCare = false;
        // _Value3List[in1.id]._dontCare = false;
        // QQ.push(in0);
        // QQ.push(in1);
        if(!values_ref[in0.id] ^ in0.cp){
          _Value3List[in0.id]._bit = values_ref[in0.id];
          _Value3List[in0.id]._dontCare = false;
          QQ.push(in0);
        }
        else if(!values_ref[in1.id] ^ in1.cp){
          _Value3List[in1.id]._bit = values_ref[in1.id];
          _Value3List[in1.id]._dontCare = false;
          QQ.push(in1);
        }
        else{
          _Value3List[in0.id]._bit = true;
          _Value3List[in1.id]._bit = true;
          _Value3List[in0.id]._dontCare = false;
          _Value3List[in1.id]._dontCare = false;
          QQ.push(in0);
          QQ.push(in1);
        ...
        const V3NetId& known = _Value3List[in0.id]._dontCare? in1 : in0;
        const V3NetId& unknown = _Value3List[in0.id]._dontCare? in0 : in1;
        if(_Value3List[known.id]._bit ^ known.cp){
          _Value3List[unknown.id]._bit = unknown.cp;
          _Value3List[unknown.id]._dontCare = false;
          QQ.push(unknown);
        }
      }
    }
  ...
  for(unsigned i=0; i<_L; ++i) c->_latchValues[i] = _Value3List[_ntk->getLatch(i).id];
  return c;
}
...
bool* V3SvrPDRSat::ternarySimInit(Cube* c) {
  ...
  //for inputs
  return getSATInputAssignment();
}

bool* V3SvrPDRSat::getSATInputAssignment() {
  bool* ii = new bool[_I];
